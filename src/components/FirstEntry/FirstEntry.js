...
const FirstEntry = ({ handleShowToast }) => {
  ...
  const fetchSongs = async (userId) => {
    try {
      const response = await axios.get(`http://radiowezel.duckdns.org:8080/getsongs`, {
        params: {
          userId: userId,
        },
      });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(`http://radiowezel.duckdns.org:8080/newuser`);
    ...
    const checkIsWebActive = async () => {
      try {
        const response = await axios.get(
          `http://radiowezel.duckdns.org:8080/isvotingactive`
        );
  ...
  const handleLogin = async (pinValue) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://radiowezel.duckdns.org:8080/login`,
        `${userPinJson}`,
        {
          headers: {
            "Content-Type": "application/json",
